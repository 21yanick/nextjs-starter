...
export const PAYMENT_METHODS = {
  swiss: ['card', 'twint'], // Immediate payments only for bookings
} as const;
...
export function getBusinessConfig() {
  const businessModel = (process.env.BUSINESS_MODEL as BusinessModel) || 'booking';
...
export function getPaymentMethodsForCheckout(businessModel?: BusinessModel) {
  ...
  // Swiss-only payment methods - immediate payments only for bookings
